#########################################################################################
# This is the test menu screen that the user will first see. It is run on the boot via a
...
create_label("TEST MODE", 120, 30, 50, red)
...
create_button("4", 375, 255, 55, 95, white)


def button(number):
    if number == 1:
        pygame.quit()
        page=os.environ["MENUDIR"] + "listening_menu_test.py"
        os.execvp("python", ["python", page])
        sys.exit()

    if number == 2:
        subprocess.call("crack_wpa.sh")
        pygame.quit()
        page=os.environ["MENUDIR"] + "listening_menu_test.py"
        os.execvp("python", ["python", page])
        sys.exit()
